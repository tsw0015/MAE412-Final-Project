...
Create_Full_Speed=0.35;          % The highest speedQ the robot can travel. (Max is 0.5m/s)
Create_Min_Speed=0.2;          % The minimum speed in order for the robot to move (m/s)
avoiding = false;               % Set to true when avoiding an obstacle
avoidingState = 1;              % 1 = turn, 2 = drive forward
avoidAngleToTurn = deg2rad(25);  % Initial value for angle to turn during avoid (radians)
avoidDriveDistance = 0.45;       % Distance to drive during avoid (meters)
avoidTriggerDistance = 100;      % Distance from front range finder (mm)
goalX = 4.2;                   % Goal x location (meters)
goalY = 0.5;                   % Goal y location (meters)
