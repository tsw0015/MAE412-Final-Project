...
Tend=605;                                    % Was 60 seconds;
Total_Steps=Tend/Ts_Desired;                 % The total number of time steps;
Create_Full_Speed=0.6;                       % The highest speed the robot can travel. (Max is 0.5m/s)
Create_Min_Speed=0.1;                       % The minimum speed in order for the robot to move (m/s)
...
avoidAngleToTurn = avoidAngleToTurnRight;    % Initial value for angle to turn during avoid (radians)
avoidDriveDistance = 0.5;                    % Distance to drive forward during avoid (meters)
avoidBackupDistance = -0.2;                   % Distance to drive backward during avoid (meters)
avoidTriggerDistance = 150;                  % Distance from front range finder (mm)
%turnCount = 0;                                % Number of turns after obstacle detected
goalX = 4.2;                                % Goal x location (meters)
goalY = .3;                                % Goal y location (meters)
biasRemovalCounts = 5/Ts_Desired;           % Steps to wait for bias removal
...
%     fprintf(S_Hokuyo,'GD0044072500');           % request a new Lidar scan
    ...
    elseif i==biasRemovalCounts
        ...
        SD.X(i)=0.457;                                                        % Initial Robot X Position
        SD.Y(i)=1.981; 
        P_Bias = mean(SD.P(1:i));
        Q_Bias = mean(SD.Q(1:i));
        R_Bias = mean(SD.R(1:i));
    else                                                                      % If i>1
        ...
        if avoiding
            switch avoidingState
                case 1 % back up
                    ...
                    avoiding = true;              
                case 2 % turn
    %               if turnCount==2 % big turn
    %                    [avoidDone, driveData] = TurnAngle(3*avoidAngleToTurn, S_Create, SD, driveData, i);
    %                    turnCount = 0;
    %                else 
...
%                    end
                ...
                if yawError > -pi/2 && yawError < pi/2
                    forwardSpeed = Create_Full_Speed*0.8*cos(yawError);
